# Patch Priority Radar

Prioritize patches by exploitability, asset criticality and exposure. Part of the Zion Tech Group App Network.

**Live app:** https://ziontechgroup.com/patch-priority-radar/

## About
Patch Priority Radar is a free, browser-based tool from the [Zion App Network](https://ziontechgroup.com/apps/) (400+ open-source AI & IT ops apps). Category: **SLA, Incidents & SRE**.

## Related apps in the network
- [AI API Health Monitor](https://ziontechgroup.com/ai-api-health-monitor/) — API health monitoring ([repo](https://github.com/Zion-support/ai-api-health-monitor))
- [AI Backup Integrity](https://ziontechgroup.com/ai-backup-integrity/) — backup verification ([repo](https://github.com/Zion-support/ai-backup-integrity))
- [AI Incident Timeline](https://ziontechgroup.com/ai-incident-timeline/) — incident timeline reconstruction ([repo](https://github.com/Zion-support/ai-incident-timeline))
- [Incident Runbook](https://ziontechgroup.com/incident-runbook/) — AI incident runbook generator ([repo](https://github.com/Zion-support/incident-runbook))
- [Site Uptime Watchdog](https://ziontechgroup.com/site-uptime-watchdog/) — uptime and performance monitoring ([repo](https://github.com/Zion-support/site-uptime-watchdog))
- [K8s Pod Security Scanner](https://ziontechgroup.com/k8s-pod-security-scanner/) — scan pods against Pod Security Standards ([repo](https://github.com/Zion-support/k8s-pod-security-scanner))

## Part of the Zion App Network
🌐 [Network Index](https://github.com/Zion-support/zion-network/blob/main/APPS_NETWORK.md) · [MSP & Partner Ecosystem](https://github.com/Zion-support/zion-network/blob/main/network/msp-partner-ecosystem.md) · [Status](https://zion-support.github.io/zion-status/) · [Plans](https://zion-support.github.io/plans/) · [Portal](https://zion-support.github.io/zion-portal/)

## Links
- 🌐 Homepage: https://ziontechgroup.com/
- 🗂️ Full directory: https://ziontechgroup.com/apps/ · [GitHub catalog](https://github.com/Zion-support/zion-support.github.io/tree/main/apps)
- 💼 Services: https://ziontechgroup.com/services/ · Discovery $99: https://ziontechgroup.com/discovery/

---
Zion Tech Group · Middletown, DE · kleber@ziontechgroup.com
